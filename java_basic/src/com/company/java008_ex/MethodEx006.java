...
public class MethodEx006 {
	public static void who_are_you(String [][] users){  //아이디를입력받아서 나라조회 
		...
		//if ( 입력받은 id가 "bbb" 같니? ) { bbb는 호주사람입니다.  }
		//if ( 입력받은 id가 "ccc" 같니? ) { ccc는 중국사람입니다.  }
	...
	public static void who_pass_change(String [][] users){ // 아이디,비밀번호가 맞으면 비밀번호 바꾸기
		...
		// 처리
		// 1)사용자 인증1 - if(입력받은 아이디와 "aaa" / 입력받은 비번과 111 같다면){ 해당유저  0층 }
		//             -  if(입력받은 아이디와 "bbb" / 입력받은 비번과 222 같다면){ 해당유저  1층 }
		// 1)사용자 인증2 - if(temp_id.equals((users[0][0])) &&  temp_pass.equals((users[0][1]))){ 해당유저  0층 }
		//             -  if(temp_id.equals((users[1][0])) &&  temp_pass.equals((users[1][1]))){ 해당유저  1층 }
