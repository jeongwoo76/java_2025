...
public class ArrayEx008 {
	...
	// ver-1 아이언맨의 평균이 60점 이상이면 합격, 아니면 불합격
	// ver-2 if(아이언맨의 평균이 60점 이상이면) {합격}, {아니면 불합격}
    // ver-3 if(aver[0] >=60)  {pass[0]="합격";} else {pass[0]="불합격";}
	//	if(aver[0] >=60)  {합격} else {불합격}
	//	if(aver[1] >=60)  {합격} else {불합격}
	//	if(aver[2] >=60)  {합격} else {불합격}
	for (int i=0; i<aver.length; i++) {  if(aver[i] >=60)  {pass[i]="합격";} else {pass[i]="불합격";}     }
	
	//	if(aver[0] >=95)  {장학생} else {----}
	//	if(aver[1] >=95)  {장학생} else {----}
	//	if(aver[2] >=95)  {장학생} else {----}
	for (int i=0; i<aver.length; i++) {  if(aver[i] >=95)  {jang[i]="장학생";} else {jang[i]="----";}    }
...
3.출력예시:     
   ...
   ※ 힌트1)    
    /////// 처리1 : 평균구하기  
    1-1. (국어+영어+수학)/3
   1-2.  아이언맨의 평균 =  아이언맨의 국어 + 아이언맨의 영어 + 아이언맨의 수학 
   1-3.  aver[0] = ( kor[0] + eng[0] + mat[0] )/3;
   
   1-2.  헐크의 평균 =  헐크의 국어 + 헐크의 영어 + 헐크의 수학 
